...
| 设备信息枚举(SMBIOS/WMI/注册表) | **保留** | `sailbreak info` 核心输入 |
...
CLI 等价:`sailbreak snapshot {capture,diff,restore}`(restore 限定到本工具管理的设置域)。

## 6. 设备信息枚举(VantageCoreAddin,`sailbreak info` 的数据源清单)
