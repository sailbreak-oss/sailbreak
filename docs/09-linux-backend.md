...
- **可写节点**(风扇、充电阈值、摄像头开关等):默认 root。
  ...
  `plugdev` / `sailbreak` / `power`,并把 `uaccess` 权限开放给组内成员。
...
CLI 工具 `sailbreak` 应先探测 EPP 可用性,只在老内核(< 5.8)回退到 `energy_perf_bias`。
...
实现者可以在 `sailbreak` daemon 中实现一个**轻量级调度器**,替代
...
2. 键码映射到 `sailbreak` 命令:
   - `KEY_BRIGHTNESSUP` → `sailbreak display brightness +1`
   - `KEY_KBDILLUMUP` → `sailbreak keyboard backlight +1`
...
将以下规则写到 `/etc/udev/rules.d/90-sailbreak.rules`:
...
通过 polkit action 放行。写入 `/etc/polkit-1/actions/org.sailbreak.admin.policy`:

```xml
<?xml version="1.0" encoding="UTF-8"?>
<policyconfig>
  <vendor>Sailbreak Project</vendor>
  <action id="org.sailbreak.acpi-call">
  ...
  <action id="org.sailbreak.modprobe">
...
```bash
# 创建 sailbreak 用户组(可选,也可以复用 plugdev)
sudo groupadd sailbreak
sudo usermod -aG sailbreak,plugdev,dialout $USER

# 加载内核模块(首次)
sudo modprobe acpi_call ideapad_laptop intel_rapl_common intel_lpmd
sudo tee /etc/modules-load.d/sailbreak.conf <<< "acpi_call ideapad_laptop intel_rapl_common intel_lpmd"
```
